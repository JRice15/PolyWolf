...
/**
 * AutoStarter クラスを一部改変したクラスです。
 */
public class MyAutoStarter {
    
    /** 実行開始時刻 */
    private LocalDateTime startTime;
    // private DateTimeFormatter dtf = DateTimeFormatter.ofPattern("HH:mm:ss\td-'th'\tMMMM").withLocale(Locale.ENGLISH);
    private DateTimeFormatter dtf_forLastLine = DateTimeFormatter.ofPattern("HH:mm:ss\tyyyy/MM/dd");
    private DateTimeFormatter dtf_forFileName = DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm-ss");
    ...
    public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, IOException {
        if (args.length == 0) {
            System.err.println("Usage:" + MyAutoStarter.class.getName() + " initFileName");
            return;
        }
        
        MyAutoStarter ssbi = new MyAutoStarter(args[0]);
        ssbi.start();
        ssbi.result();
        System.exit(1);
    }
     ...
     * @throws IOException
     */
    public MyAutoStarter(String fileName) throws IOException {
...
//			processMap.put(teamName, process);
    ...
    public void result() throws IOException {
        // numbers で開くと，いい感じに見ることができる。
        // File logFile = new File(String.format("%s/total.txt", logDirName)); // <- txtファイル
        // File logFile = new File(String.format("%s/total_%s.tsv", logDirName, startTime.format(dtf_forFileName))); // <- tsvファイル
        File logFile = new File(String.format("%s/result.tsv", logDirName)); // <- tsvファイル
...
//		for(String name:new TreeSet<>(roleAgentMap.keySet())){
            ...
            for (Role role : Role.values()) {
                ...
                filewriter.write(String.format("%d/%d\t", winCounterMap.get(name).get(role), roleCounterMap.get(name).get(role)));
                win += winCounterMap.get(name).get(role);
                cnt += roleCounterMap.get(name).get(role);
            }
            System.out.printf("%.3f\n", win / cnt);
            filewriter.write(String.format("%.3f\n", win / cnt));
        }
        
        filewriter.write("\nGames\t" + gameNum
                + "\nStart\t" + startTime.format(dtf_forLastLine)
                + "\nFinish\t" + (LocalDateTime.now()).format(dtf_forLastLine) + "\n");
