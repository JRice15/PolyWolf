...
ALL_VISIBILITY = ["talk", "vote", "execute"]
ROLE_VISIBILITY = {
    "BODYGUARD": ["guard"],
    ...
    "POSSESSED": [],
    "VILLAGER": [],
    "WEREWOLF": ["whisper", "attack", "attack_result", "attackVote"],
}
        ...
        for row in log_reader:
            if len(row):
                ...
                # actions that should result in multiple rows
                    ...
                    divine_row = ["divine", 1, np.nan, parsed_row['target_id']]
                    ...
                    result = "ATTACKED" if parsed_row["attack_success"] == "true" else "GUARDED"
                    result_row = ["attack_result", 0, np.nan, result]
                    all_rows += [attack_row, result_row]

                # other random results
                elif action == "guard":
                    row = [action, 1, np.nan, parsed_row["target_id"]]
                    all_rows.append(row)
...
def one_hot_encode(df, sparse=False):
    """
